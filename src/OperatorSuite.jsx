...
// ── MARK SIMULATOR LIVE ──────────────────────────────────────
// One-action cockpit write: updates ARCHETYPES + PROTOTYPES + INFRASTRUCTURE_GALLERY
// triggered from Snapshot module after a simulator deploys.

function MarkSimulatorLive({ assets, setArchetypes, setPrototypes, setGallery }) {
  ...
  const archetypes = assets?.archetypes || [];
  const prototypes = assets?.prototypes || [];
  const gallery    = assets?.gallery    || [];

  // Auto-fill IDs when archetype is selected
  const onArchetypeSelect = (archetypeId) => {
    const proto = prototypes.find(p => p.archetypeId === archetypeId);
    const gal   = gallery.find(g => g.archetypeId === archetypeId);
    ...
    setForm(f => ({
      ...
      galleryId:    gal?.id      || "",
      publicTitle:  arch?.name   || "",
    }));
  };

  const save = async () => {
    if (!form.liveUrl || !form.archetypeId) {
      setError("Archetype and Live URL are required.");
      return;
    }
    setSaving(true);
    setError("");
    ...
    try {
      ...
      // 1. Update ARCHETYPES — lifecycle_state → Deployed, frontend_demo_url
      if (form.archetypeId) {
        writes.push(
          adapter.update("ARCHETYPES", form.archetypeId, {
            lifecycle_state:   "Deployed",
            frontend_demo_url: form.liveUrl,
            updated_at:        now,
          })
        );
      }

      // 2. Update PROTOTYPES — build_stage → Live, live_url, repo_path
      if (form.prototypeId) {
        writes.push(
          adapter.update("PROTOTYPES", form.prototypeId, {
            build_stage:    "Live",
            live_url:       form.liveUrl,
            repo_path:      form.repoPath,
            last_tested_at: now.split("T")[0],
            updated_at:     now,
          })
        );
      }

      // 3. Update INFRASTRUCTURE_GALLERY — published_status → Published, frontend_demo_url
      if (form.galleryId) {
        writes.push(
          adapter.update("INFRASTRUCTURE_GALLERY", form.galleryId, {
            frontend_demo_url: form.liveUrl,
      ...
      writes.push(
        adapter.append("ACTIVITY_LOG", {
          log_id:      "LOG-" + Date.now().toString(36),
          timestamp:   now,
          entry_type:  "Deployment",
          entity_type: "Archetype",
          entity_id:   form.archetypeId,
          summary:     `Simulator marked Live: ${form.publicTitle || form.archetypeId} — ${form.liveUrl}`,
          operator:    "hirrok",
        })
      );
      ...
      setArchetypes(prev => prev.map(a =>
        a.id === form.archetypeId
          ? { ...a, lifecycle: "Deployed", demoUrl: form.liveUrl }
          : a
      ));
      setPrototypes(prev => prev.map(p =>
        p.id === form.prototypeId
          ? { ...p, stage: "Live", liveUrl: form.liveUrl }
          : p
      ));
      setGallery(prev => prev.map(g =>
        g.id === form.galleryId
          ? { ...g, published: true, demoUrl: form.liveUrl }
          : g
      ));

      setDone(true);
      setSaving(false);
      setTimeout(() => { setOpen(false); setDone(false); setForm({ archetypeId:"", prototypeId:"", galleryId:"", liveUrl:"", repoPath:"", publicTitle:"" }); }, 2000);
  ...
  return (
    <>
      ...
      <div style={{ background:B.raised, border:`1px solid ${B.border}`, borderRadius:10, padding:14 }}>
        <SLabel>Simulator Deployment</SLabel>
        <p style={{ fontSize:12, color:B.dim, marginBottom:12, lineHeight:1.5 }}>
          Mark a simulator live — updates ARCHETYPES, PROTOTYPES, and INFRASTRUCTURE_GALLERY in one action.
        </p>
        <GBtn label="⚡ Mark Simulator Live" onClick={() => setOpen(true)} on />
      </div>

      {/* Modal */}
      {open && (
        <div style={{
          position:"fixed", inset:0, background:"rgba(14,28,26,0.7)", zIndex:200,
          display:"flex", alignItems:"center", justifyContent:"center", padding:"1rem",
        }} onClick={e => { if(e.target === e.currentTarget) setOpen(false); }}>
          <div style={{
            background:B.surface, borderRadius:12, padding:24, width:"100%", maxWidth:480,
            border:`1px solid ${B.border}`, maxHeight:"90vh", overflowY:"auto",
          }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
              <p style={{ fontFamily:F.mono, fontSize:10, letterSpacing:2, color:B.teal, textTransform:"uppercase" }}>
                Mark Simulator Live
              </p>
              <button onClick={() => setOpen(false)} style={{ background:"none", border:"none", cursor:"pointer", color:B.dim, fontSize:18, lineHeight:1 }}>×</button>
            </div>

            {done ? (
              <div style={{ background:B.gbg, border:`1px solid rgba(10,138,64,.3)`, borderRadius:8, padding:16, textAlign:"center" }}>
                <p style={{ fontFamily:F.mono, fontSize:11, color:B.green, letterSpacing:1 }}>✓ SIMULATOR MARKED LIVE</p>
                <p style={{ fontSize:12, color:B.dim, marginTop:6 }}>ARCHETYPES · PROTOTYPES · GALLERY updated</p>
              </div>
            ) : (
              <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                ...
                {/* Archetype selector */}
                <div>
                  <p style={{ fontFamily:F.mono, fontSize:9, letterSpacing:2, color:B.dim, textTransform:"uppercase", marginBottom:5 }}>
                    Archetype *
                  </p>
                  <select
                    value={form.archetypeId}
                    onChange={e => onArchetypeSelect(e.target.value)}
                    style={{ width:"100%", background:B.raised, border:`1px solid ${B.border}`, borderRadius:4, padding:"8px 10px", fontSize:13, color:B.ink, fontFamily:F.body }}
                  >
                    ...
                    {archetypes.length === 0 && (
                      <option disabled>No archetypes loaded — spine may be offline</option>
                    )}
                  </select>
                </div>
                ...
                {form.prototypeId && (
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                    {[["Prototype ID", form.prototypeId], ["Gallery ID", form.galleryId || "—"]].map(([l,v]) => (
                      <div key={l} style={{ background:B.raised, border:`1px solid ${B.border}`, borderRadius:4, padding:"8px 10px" }}>
                        <p style={{ fontFamily:F.mono, fontSize:8, color:B.muted, letterSpacing:1, textTransform:"uppercase", marginBottom:3 }}>{l}</p>
                        <p style={{ fontFamily:F.mono, fontSize:10, color:B.teal }}>{v}</p>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="https://hirrok.github.io/landmark-restaurant-simulator/"
                    style={{ width:"100%", background:B.raised, border:`1px solid ${B.border}`, borderRadius:4, padding:"8px 10px", fontSize:13, color:B.ink, fontFamily:F.body }}
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="hirrok/landmark-restaurant-simulator"
                    style={{ width:"100%", background:B.raised, border:`1px solid ${B.border}`, borderRadius:4, padding:"8px 10px", fontSize:13, color:B.ink, fontFamily:F.body }}
                  />
                </div>

                {error && (
                  <p style={{ fontFamily:F.mono, fontSize:10, color:B.red, background:B.rbg, border:`1px solid rgba(204,34,34,.3)`, borderRadius:4, padding:"8px 10px" }}>
                    ⚠ {error}
                  </p>
                )}
                ...
                <div style={{ display:"flex", gap:8, paddingTop:4 }}>
                  <button
                    ...
                    disabled={saving}
                    style={{
                      flex:1, background: saving ? B.muted : B.teal, color:"#fff",
                      fontFamily:F.mono, fontSize:10, letterSpacing:".1em", textTransform:"uppercase",
                      padding:"10px 16px", borderRadius:6, border:"none", cursor: saving ? "wait" : "pointer",
                    }}
                  >
                    {saving ? "Saving…" : "Mark Live — Write to Spine"}
                  </button>
                  <button
                    onClick={() => setOpen(false)}
                    style={{ background:B.raised, border:`1px solid ${B.border}`, color:B.dim, fontFamily:F.mono, fontSize:10, padding:"10px 14px", borderRadius:6, cursor:"pointer" }}
                  >
                    Cancel
                  </button>
                </div>

                <p style={{ fontFamily:F.mono, fontSize:9, color:B.muted, textAlign:"center", letterSpacing:1 }}>
                  Writes to ARCHETYPES · PROTOTYPES · INFRASTRUCTURE_GALLERY · ACTIVITY_LOG
                </p>
              </div>
            )}
          </div>
...
function Snapshot({ leads, assets, nav, setArchetypes, setPrototypes, setGallery }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
      ...
      <MarkSimulatorLive assets={assets} setArchetypes={setArchetypes} setPrototypes={setPrototypes} setGallery={setGallery} />
    </div>
  );
}
...
function Leads({ leads, setLeads, ai }) {
  ...
  const [wonId,    setWonId]    = useState(null);   // prospect being converted
  ...
  // ── MARK WON ────────────────────────────────────────────────
  const markWon = (id, lead) => {
    // Set defaults from spine record
    setWonId(id);
    setWonForm({
  ...
  const submitWon = async () => {
    ...
    const payload = {
      prospectId:      wonId,
      setupFee:        Number(wonForm.setupFee)        || 0,
      monthlyRetainer: Number(wonForm.monthlyRetainer) || 0,
      templateFamily:  wonForm.templateFamily || "",
      maintenanceTier: "Standard",
      contactName:     wonForm.contactName || "",
      domain:          wonForm.domain || "",
      launchDate:      wonForm.launchDate || "",
      notes:           wonForm.notes || "",
      depositPaid:     wonForm.depositPaid || false,
    };

    const now = new Date().toISOString();

    // Write 1: update PROSPECTS stage to Conversion
    await adapter.update("PROSPECTS", wonId, {
      stage:        "Conversion",
      record_state: "ACTIVE",
      next_action:  "Onboard — collect deposit, set up accounts",
      updated_at:   now,
    });

    // Write 2: append CLIENTS record
    const clientId = "cli-" + Date.now().toString(36);
    const lead = leads.find(l => l.id === wonId) || {};
    await adapter.append("CLIENTS", {
      client_id:        clientId,
      prospect_id:      wonId,
      business_name:    lead.name || "",
      template_family:  payload.templateFamily || lead.templateFamily || "",
      live_url:         "PENDING",
      backend_url:      "PENDING",
      ...
      contact_phone:    "",
      contact_email:    "",
      notes:            payload.notes || "Converted from " + wonId,
      created_at:       now,
      updated_at:       now,
    });

    // Write 3: log setup fee
    if (payload.setupFee > 0) {
      await adapter.append("REVENUE", {
        rev_id:       "REV-" + Date.now().toString(36),
        created_at:   now,
        client_id:    clientId,
        business_name: lead.name || "",
        type:          "Setup",
        amount:        payload.setupFee,
        status:        payload.depositPaid ? "Paid" : "Invoiced",
        period:        now.split("T")[0].substring(0, 7),
        notes:         "Setup fee on conversion. Prospect: " + wonId,
      });
    }

    // Write 4: log MRR
    if (payload.monthlyRetainer > 0) {
      await adapter.append("REVENUE", {
        rev_id:       "REV-" + (Date.now()+1).toString(36),
        created_at:   now,
        client_id:    clientId,
        business_name: lead.name || "",
        type:          "Maintenance",
        amount:        payload.monthlyRetainer,
        status:        "Invoiced",
        period:        now.split("T")[0].substring(0, 7),
        notes:         "Monthly maintenance retainer. Starts on launch.",
      });
    }

    // Write 5: activity log
    await adapter.append("ACTIVITY_LOG", {
      log_id:      "LOG-" + Date.now().toString(36),
      timestamp:   now,
    ...
    // Optimistic UI — move to Conversion stage
    setLeads(prev => prev.map(l => l.id === wonId ? { ...l, status:"Conversion", recordState:"ACTIVE" } : l));
    setWonSaving(false);
    setWonDone(true);
    setTimeout(() => { setWonId(null); setWonDone(false); }, 2500);
  };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
      ...
      {wonId && (
        <div style={{
          position:"fixed", inset:0, background:"rgba(14,28,26,0.75)", zIndex:200,
          display:"flex", alignItems:"center", justifyContent:"center", padding:"1rem",
        }} onClick={e => { if(e.target === e.currentTarget && !wonSaving) setWonId(null); }}>
          <div style={{ background:B.surface, borderRadius:12, padding:24, width:"100%", maxWidth:460,
            border:`1px solid ${B.border}`, maxHeight:"90vh", overflowY:"auto" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
              <p style={{ fontFamily:F.mono, fontSize:10, letterSpacing:2, color:B.green, textTransform:"uppercase" }}>
                ✓ Mark Won — Convert to Client
              </p>
              <button onClick={() => setWonId(null)} style={{ background:"none", border:"none", cursor:"pointer", color:B.dim, fontSize:18 }}>×</button>
            </div>

            {wonDone ? (
              <div style={{ background:B.gbg, border:`1px solid rgba(10,138,64,.3)`, borderRadius:8, padding:16, textAlign:"center" }}>
                <p style={{ fontFamily:F.mono, fontSize:11, color:B.green, letterSpacing:1 }}>✓ CONVERSION COMPLETE</p>
                <p style={{ fontSize:12, color:B.dim, marginTop:6 }}>PROSPECTS · CLIENTS · REVENUE · ACTIVITY_LOG updated</p>
              </div>
            ) : (
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                {[
                ...
                ].map(([label, key, inputType, ph]) => (
                  <div key={key}>
                    <p style={{ fontFamily:F.mono, fontSize:9, letterSpacing:2, color:B.dim, textTransform:"uppercase", marginBottom:4 }}>{label}</p>
                    <input type={inputType} value={wonForm[key] || ""} placeholder={ph}
                      onChange={e => setWonForm(prev => ({ ...prev, [key]: inputType==="number" ? Number(e.target.value) : e.target.value }))}
                      style={{ width:"100%", background:B.raised, border:`1px solid ${B.border}`, borderRadius:4,
                        padding:"8px 10px", fontSize:13, color:B.ink, fontFamily:F.body, outline:"none" }} />
                  </div>
                ))}
                <div>
                  <p style={{ fontFamily:F.mono, fontSize:9, letterSpacing:2, color:B.dim, textTransform:"uppercase", marginBottom:4 }}>Notes</p>
                  <textarea value={wonForm.notes || ""} rows={2}
                    onChange={e => setWonForm(prev => ({ ...prev, notes: e.target.value }))}
                    style={{ width:"100%", background:B.raised, border:`1px solid ${B.border}`, borderRadius:4,
                      padding:"8px 10px", fontSize:13, color:B.ink, fontFamily:F.body, outline:"none", resize:"vertical" }} />
                </div>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  <input type="checkbox" id="depositCheck" checked={wonForm.depositPaid || false}
                    onChange={e => setWonForm(prev => ({ ...prev, depositPaid: e.target.checked }))}
                    style={{ width:"auto" }} />
                  <label htmlFor="depositCheck" style={{ fontSize:12, color:B.ink }}>Deposit already received</label>
                </div>
                <div style={{ display:"flex", gap:8, paddingTop:4 }}>
                  <button onClick={submitWon} disabled={wonSaving} style={{
                    flex:1, background:wonSaving ? B.muted : B.green, color:"#fff",
                    fontFamily:F.mono, fontSize:10, letterSpacing:".1em", textTransform:"uppercase",
                    padding:"10px 16px", borderRadius:6, border:"none", cursor:wonSaving?"wait":"pointer",
                  }}>{wonSaving ? "Converting…" : "Confirm Conversion"}</button>
                  <button onClick={() => setWonId(null)} style={{ background:B.raised, border:`1px solid ${B.border}`,
                    color:B.dim, fontFamily:F.mono, fontSize:10, padding:"10px 14px", borderRadius:6, cursor:"pointer" }}>
                    Cancel
                  </button>
                </div>
                <p style={{ fontFamily:F.mono, fontSize:9, color:B.muted, textAlign:"center", letterSpacing:1 }}>
                  Writes to PROSPECTS · CLIENTS · REVENUE · ACTIVITY_LOG
                </p>
              </div>
            )}
          </div>
          ...
          <div key={l.id}
            ...
            {expanded === l.id && (
              <div style={{ marginTop:10, paddingTop:10, borderTop:`1px solid ${B.border}` }}>
                ...
                {l.status === "Proposal" && (
                  <div style={{ marginTop:10, paddingTop:10, borderTop:`1px solid ${B.border}` }}>
                    <p style={{ fontFamily:F.mono, fontSize:9, color:B.green, letterSpacing:1, marginBottom:6 }}>CONVERSION READY</p>
                    <button
                      onClick={e => { e.stopPropagation(); markWon(l.id, l); }}
                      style={{
                        background:B.green, color:"#fff", border:"none", cursor:"pointer",
...
export default function FoundryOps() {
  ...
  const MODULE_MAP = {
    snapshot:  <Snapshot  leads={leads} assets={assets} nav={nav} setArchetypes={setArchetypes} setPrototypes={setPrototypes} setGallery={setGallery} />,
